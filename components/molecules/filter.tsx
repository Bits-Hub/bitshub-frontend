...
export default function Filter({onFilterChange}: {onFilterChange: any}) {
    const object = [
        {
          id: 1,
          name: 'macbook'
            ...
            {/* {categories?.data?.map((item) => {
              return (
                <div className="flex items-center" key={item.id}>
                  <input type="checkbox" id="cat-1" className="text-primary focus:ring-0 rounded-sm cursor-pointer" checked={filterOption === item.id} onChange={() => handleChange(item.id)} />

                  <label htmlFor="cat-1" className="text-gray-600 ml-3 cursor-pointer lowercase">
                    {item.category}
                  </label>
                  commented out in the old code
              ...
              object.map((item) => {
                return (
                  <div className="flex items-center" key={item.name}>
                    <input type="checkbox" id="cat-1" className="text-primary focus:ring-0 rounded-sm cursor-pointer" checked={filterOption === item.name} onChange={() => handleChange(item.name)} />
